...
contains
  ...
  subroutine dyn_shallow_u(nx,ny,x_rho,y_rho,x_u,y_u,x_v,y_v,f,&
       ...
       & tau_x,u,v,p,u_past,u_next,cn,obn,dt)
    implicit none
    integer,intent(in) :: nx,ny
    real(idx),intent(in) :: x_rho(0:nx+1), y_rho(0:ny+1), x_u(1:nx+1), y_u(0:ny+1), x_v(0:nx+1),y_v(1:ny+1)
    ...
    do iy = 1,ny
       do ix = 2,nx
          ...
          ! U equation
          !=================================
          drag_u = -1.0_idx * u(ix,iy) * (A / (cn**2)+damp_u(ix,iy))
  ...
  subroutine dyn_shallow_v(nx,ny,x_rho,y_rho,x_u,y_u,x_v,y_v,f,&
       & mask_v,mask_phi_v,damp_v,nu,tau_y,u,v,p,v_past,v_next,cn,obn,dt)
    implicit none
    integer,intent(in) :: nx,ny
    real(idx),intent(in) :: x_rho(0:nx+1), y_rho(0:ny+1), x_u(1:nx+1), y_u(0:ny+1), x_v(0:nx+1),y_v(1:ny+1)
    real(idx),intent(in) :: f(0:ny+1)
    ...
    do iy = 2,ny
       do ix = 1,nx
          ...
          ! V equation
          !=================================
          drag_v = -1.0_idx * v(ix,iy) * (A / (cn**2)+damp_v(ix,iy))
