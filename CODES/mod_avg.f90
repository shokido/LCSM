...
contains
  ...
  subroutine write_avg_2d(fname,nx,ny,irec,uw,vw,mask_p,missing_value,avg_count)
    use ncdf_write
    implicit none
    character(len=*),intent(in) :: fname
    ...
    do iy=0,ny+1
       do ix=0,nx+1
          if (mask_p(ix,iy) .eq. 0.0_idx) then
             ...
             uw_2d(ix,iy,1)=uw(ix,iy)/avg_count
             vw_2d(ix,iy,1)=vw(ix,iy)/avg_count
          end if
          uw(ix,iy)=0.0_idx; vw(ix,iy)=0.0_idx
       end do
    end do
    call writenet_wv(trim(fname),"uw",1,nx+2,1,ny+2,irec,irec,uw_2d(0:nx+1,0:ny+1,1:1))
    call writenet_wv(trim(fname),"vw",1,nx+2,1,ny+2,irec,irec,vw_2d(0:nx+1,0:ny+1,1:1))
  end subroutine write_avg_2d
end module mod_avg
