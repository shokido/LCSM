program do_flood_LCSM
  ...
  ! Get coordinate variable
    call get_dimsize(trim(fname_grid),"x_p",nx_rho)
   call get_dimsize(trim(fname_grid),"y_p",ny_rho)
   call get_variable(trim(fname_grid),"x_p",(/1/),(/nx_rho/),x_1d)
   call get_variable(trim(fname_grid),"y_p",(/1/),(/ny_rho/),y_1d)
   allocate(mask_rho(nx_rho,ny_rho))
   call get_variable(trim(fname_grid),"mask_p",(/1,1/),(/nx_rho,ny_rho/),mask_rho)
  ...
  ! Input file
  do ifile =1,nfile
     call get_dimsize(fname_in(ifile),lonname,nlon_in)
     call get_variable(fname_in(ifile),lonname,(/1/),(/nlon_in/),lon_in)
     call get_attribute(fname_in(ifile),lonname,"units",lonunits)
     call get_dimsize(fname_in(ifile),latname,nlat_in)
     call get_variable(fname_in(ifile),latname,(/1/),(/nlat_in/),lat_in)
     call get_attribute(fname_in(ifile),latname,"units",latunits)
     write(*,*) trim(fname_in(ifile)),trim(levname)
     call get_dimsize(fname_in(ifile),levname,nlev_in)
     call get_variable(fname_in(ifile),levname,(/1/),(/nlev_in/),lev_in)
     call get_attribute(fname_in(ifile),levname,"units",levunits)
     call get_dimsize(fname_in(ifile),timename,ntime_in)
     call get_variable(fname_in(ifile),timename,(/1/),(/ntime_in/),time_in)
     ...
     do itime =1,ntime_in
        ...
        do ilev=1,nlev_in
           ...
           mask_wet=0.0_idx
           mask_bathy=0.0_idx
           call get_variable(fname_in(ifile),varname(ifile),(/1,1,ilev,itime/),(/nlon_in,nlat_in,ilev,itime/),data_in)
           data_out=data_in
           ! Find mask
           ...
           call writenet_wv(fname_out(ifile),varname(ifile),(/1,1,ilev,itime/),(/nlon_in,nlat_in,ilev,itime/),data_out)
        end do
      end do
